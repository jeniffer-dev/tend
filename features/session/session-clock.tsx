...
export function SessionClock({ session }: { session: SessionFixture }) {
  return (
    <Card className="flex flex-col items-center gap-2.5 px-5 py-8">
      ...
      {/* Two lines are reserved whether or not this state needs them.
          The clock note is one of the three things FR-019 permits to differ
          between the states, and the running note is one line where the
          ...
          shift SC-007 forbids and the quickstart warns to look for. Two
          lines is the measured maximum at 320px, 390px and 720px. */}
      <span className="min-h-10 text-center text-sm text-muted-foreground text-pretty">
        {session.clockNote}
      </span>
    </Card>
  );
