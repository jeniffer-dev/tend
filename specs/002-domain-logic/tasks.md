...
- [ ] T025a [US1] Render the consequence line in `features/session/picker-list.tsx`, above the primary action, when a session is running on a task in another area (FR-015c). Nothing when it runs in this area — that is a switch, and the 001 footer note already says so. The screen is handed the finished sentence or nothing, never the running session to phrase itself from. Depends on T002a
