...
- [X] T021a [US1] Give the `attended` treatment in `features/home/area-card.tsx` the past-rhythm card's full shape — line above, **outline** Tend below leading to the area's Picker — and drop the one-line collapse and the `opacity-55` (FR-015e, approved 2026-09-25). The line is unchanged and no string is added; the button reuses `tendAction`. Ordering is untouched: attended still sinks below the open cards (001's FR-013a). Rewrite the header comment, which still says the card "loses its Tend button" and that `past-rhythm` differs from `to-tend` by nothing else. Nothing on the card may read as disabled
- [X] T021b [P] [US1] Extend `tests/e2e/session-continuity.spec.ts` with the path FR-015e opens — close a session on Health, tap the attended card's outline Tend, start again, and assert Home reads `Attended today, {n} minutes · tending now`. Assert the attended card's Tend is visible, enabled and not faded. Covers FR-015b by tapping, FR-015e
...
- [X] T031 [US2] Rewire `app/week/page.tsx` and `features/week/week-row.tsx` — heading, rows, sessions committed and attended, all derived. An open session is already counted here and Week says nothing about it, because Week has no minutes for the omission to explain (FR-006a). **No minutes reach this screen** (FR-007): the derivation that would produce a minute total is not offered to it. **Every unarchived area is a row, daily or not** (FR-007a, superseding 001 Q3, decided 2026-09-26), and a row with no sessions reads `No sessions attended.` — the twenty-seventh string, added to `lib/copy.ts` here and linted by `tests/unit/copy.test.ts`
- [X] T032 [US2] Add `Look back on last week` to `app/week/page.tsx` — a tertiary text link beneath `Change the rhythm`, not a button, present every day (FR-019b). It passes Article III's addition test on its merits, argued in spec.md §"Why Week carries a route to last week"
- [X] T033 [US2] Rewire `app/review/page.tsx` and `features/review/review-section.tsx` — Attended before Unattended, minutes as plain figures, the week's date in the eyebrow (FR-008). Minutes sum the **closed** sessions, and an open session is named on its area's row in one of the three forms, so a short figure is explained rather than merely short (FR-006a, FR-008a). An area whose only session this week is the open one renders no figure at all, never a zero. Per FR-008b (2026-09-26): every unarchived area is listed, an unattended row is the name with **no line**, one closed session with no note keeps 001's one-line `One session, {n} minutes`, and the last note is the most recent non-empty one. `lib/fixtures.ts` stops being read here. Depends on T002a
...
- [X] T036 [US2] Update `features/home/review-entry.tsx` — the Sunday string, the Monday string, or nothing on other days (FR-019a)
...
- [X] T038 [P] [US2] Write `tests/e2e/review-reachable.spec.ts` — Review is reachable on every day of the week by at least one route, and Week's link always reaches last week. Covers FR-019a, FR-019b, FR-019c
...
- [ ] T059 **Seed 001's e2e specs, and confirm each screen still renders the same copy.** Every one of them navigates to `?seed=001` and then moves by tapping, never by a second `page.goto`. `tests/e2e/persistence.spec.ts` keeps its assertions unchanged: nothing may be written to `localStorage`, `sessionStorage`, IndexedDB or cookies (FR-023), and a reload still resets.
  ...
  **Done ahead of this task, 2026-09-25: every test that lands on `/`.** Unseeded, `/` has no areas and replaces itself with `/first-run` from an effect after mount, so `/` is a URL the page passes through, not one it stops at. `toHaveURL(/\/$/)` passed when it polled before the effect and failed when it polled after, and the set of failures changed between runs — a different handful each time, and a pass that proved nothing when it came. The back rule in `navigation.spec.ts` and the four `returns to Home` tests in `capture-inbox.spec.ts` and `week-review.spec.ts` now start from `?seed=001`. **The rule for the rest of this task**: a test that asserts it arrived at `/` must be seeded, or it is asserting a transient URL.

  **One assertion changes on purpose.** `home-picker-session.spec.ts` asserts the attended card has no Tend (001's FR-014). FR-015e superseded that on 2026-09-25, so the assertion becomes "has an outline Tend that is not faded". It is rewritten because the requirement changed, not deleted to make the suite pass. The same holds for `week-review.spec.ts`: `FR-022: four areas` expects four rows and 001's row lines, and FR-007a (2026-09-26) makes it five, with People reading `One task on the list. No sessions attended.` The Q3 test was already rewritten with T031.

  **Review joined this task at T033, 2026-09-26.** Its 001 tests in `week-review.spec.ts` and `minutes.spec.ts` passed until then only because Review read `lib/fixtures.ts`; reading the store, unseeded Review is empty, and fourteen of them (seven at each width) now fail with the rest. Seeding them is not enough on its own, because three assertions describe what FR-008b superseded: `three attended areas and one unattended` becomes four and one (People), the unattended row's `No sessions this week.` becomes no line at all, and Review's heading and notes follow the seed per spec.md §"Open". Each is rewritten to the new requirement, not deleted
