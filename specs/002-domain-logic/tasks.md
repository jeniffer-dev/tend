...
- [ ] T002 Extend `lib/copy.ts` with the **twenty-one approved** strings from spec.md §"Screen copy", character-exact. The four marked **pending approval** there are **not** added by this task: FR-027 forbids building on provisional wording, and they land in T002a once the mark is gone. Several are templates — `{area}`, `{n}`, `{names}`, `{sessions}` — and the interpolation points are named there. No user-facing string may be written inline in a component or assembled from fragments in a derivation. Covers FR-027
- [ ] T002a Add the four post-analyze strings to `lib/copy.ts` **once they are approved** — Home's `Attended today, {n} minutes · tending now` and Review's three open-session forms (spec.md §"Screen copy"). Blocks T021 and T033, and nothing else
...
- [ ] T005 Implement the reducer in `lib/state/store.ts` with the **eleven** transitions in data-model.md §"State transitions" — including **create an area** and **edit an area**, which were missing from that table until after `/speckit-analyze` and are what US4's rhythm change and US5's first area both run through. Settle the open question in spec.md §"Open" first: starting a session while one is running currently orphans the first, and Review's copy says `One still open.` in the singular. `actualMinutes` is **written once, at close, and never recalculated** (FR-010a) — a session orphaned by a closed tab has no `endedAt` for a derived figure to come from, which is why PRODUCT-SPEC §3.3 makes it a field
