...
- [ ] T034 [US2] Implement which week Review shows in `app/review/page.tsx` — with no parameter the closing week on Sunday and the week that last closed on every other day, and `?week=last` the week before the current one on every day (FR-019a, FR-019c, FR-019d). Review must state which week it is showing
