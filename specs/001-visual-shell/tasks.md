...
- [X] T017 [P] [US1] Create `features/session/picker-list.tsx` — the area's week-list tasks, each with its last-session note, first task selected on arrival (FR-017). The `Last session` row label this task originally specified was removed by T048's addition test
...
- [X] T047 Verify all ten screens on a real device at 390px and 320px per quickstart.md, using `npm run dev` (which prints a Network address; there is no `--host` flag). Article VII requires device or emulator verification; the README is right that the device is more honest
